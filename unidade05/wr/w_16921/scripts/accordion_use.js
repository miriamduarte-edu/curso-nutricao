...
accordionUse1 = {
	...
	doUpdate: function() 
	{
		for(i=0;i<window.parent.document.getElementsByClassName("cp-widget").length;i++){
			if(window.parent.document.getElementsByClassName("cp-widget")[i].children[0].children[0].contentDocument.getElementById("accordionwdgt") != null){
...
function setOptionsHeight(){
	...
	for(i=0;i<textArray.length;i++){
		if($('#btnHeader'+i).hasClass("activeBtn")){
			$('#btnHeader'+i).children(this).text(textArray[i])
		}else {
			if($('#btnHeader'+i).hasClass("header") || $('#btnHeader'+i).hasClass("unactive")){
				if($('#btnHeader'+i).height()>22 && $('#btnHeader'+i).text().length>50){
					var tempText = $('#btnHeader'+i).children(this).text().slice(0,$('#btnHeader'+i).text().length-30)
					$('#btnHeader'+i).children(this).text(tempText+"...")
					if(resizerCounter<=20){
						setOptionsHeight()
					}
				}
			}
